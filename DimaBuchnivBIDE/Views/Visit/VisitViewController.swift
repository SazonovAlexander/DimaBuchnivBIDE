...
class VisitViewController: UIViewController {
    ...
    private var selectedPatient: PatientResponse?
    ...
    func getVisit() {
        inLoad.toggle()
        visitService.fetchAllVisit(page: currentPage, count: itemsPerPage, date: searchDateTextField.text ?? "", completion: { [weak self] result in
            ...
            case .success(let visits):
                ...
                self.cellDataSource.append(contentsOf: visits)
                inLoad.toggle()
    ...
    func didTapAddButton() {
        ...
        alert.addTextField { (textField) in
            textField.placeholder = "Пациент"
            textField.addTarget(self, action: #selector(Self.didSelectPatient), for: .editingDidBegin)
        }
...
private extension VisitViewController {
    ...
    func updateVisit(visit: VisitResponse) {
        ...
        alert.addTextField { (textField) in
            textField.placeholder = "Пациент"
            textField.text = visit.patient.fio
            textField.addTarget(self, action: #selector(Self.didSelectPatient), for: .editingDidBegin)
        }
        let save = UIAlertAction(title: "Сохранить", style: .default) { [weak alert, weak self] _ in
            if let self,
    ...
    @objc
    func didSelectPatient() {
        let selectPatientViewController = SelectPatientViewController()
        selectPatientViewController.delegate = self
        selectPatientViewController.modalPresentationStyle = .pageSheet
        currentAlert?.present(selectPatientViewController, animated: true)
    }
}


//MARK: - UITableViewDelegate
extension VisitViewController: UITableViewDelegate {
...
extension VisitViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.resultButtonAction = {[weak self] in
            ...
            self.navigationController?.pushViewController(serviceVisitVC, animated: true)
        }
        return cell
    }
...
extension VisitViewController: SelectDoctorDelegate {
...
extension VisitViewController: SelectPatientDelegate {
    
    func setPatient(_ patient: PatientResponse) {
        selectedPatient = patient
        currentAlert?.textFields?[2].text = patient.fio
    }
